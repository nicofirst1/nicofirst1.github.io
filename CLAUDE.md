...
4. **The site→Substack backlink is automatic** — no manual step. **Substack reuses the site slug** (verified across all 15 posts: e.g. title "Introducing PolAI Graph…" still lives at `/p/polai-graph`), so the URL is deterministically `{substack}/p/{slug}`. The `Substack backlinks` workflow (`.github/workflows/substack-backlinks.yml`, daily + manual) **probes `{substack}/p/{slug}` for each post** (200 = live; a missing slug 302-redirects) and writes confirmed-live slugs to `_data/substack.yml`. `_layouts/blogs.html` renders the "Read & discuss this on Substack" link for any slug in that list. A per-post `substack_url:` front-matter value still overrides if ever needed. NOTE: Substack's public RSS `/feed` does **not** list imported posts, so the probe — not the feed — is the source of truth.
